 ...
 *
 * KMP算法:
 *  当2个字符串匹配时，穷举法步骤是两个指针i,j分别指向原串和模式串，当2个字符相同时指针分别往后移动，但若出现字符不匹配时，
 *  i指针回到原来的i+1位置，j指针则回到模式串的第0位置，继续匹配，这样做的效率太低，O(n*m) 并且浪费了原有一部分字符串匹配的事实（因为i和j指针都同时移动了n位)
 *  KMP算法就是解决只回退j指针，不回退i指针，大大降低时间复杂度，O(n+m)，利用了原有一部分字符串匹配的事实，将j指针尽量回退到一个有效位置
 *
 * 关于next数组求解算法可看:
